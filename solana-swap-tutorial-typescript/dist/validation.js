...
exports.validateMint = validateMint;
...
function validateSlippage(slippageBps) {
    if (typeof slippageBps !== "number" ||
        ...
        slippageBps < 0 ||
        slippageBps > 10000) {
        throw new Error(`Invalid slippage: ${slippageBps}. Slippage must be a number between 0 and 10000.`);
    }
}
function validateRetries(maxRetries) {
    if (!Number.isInteger(maxRetries) || maxRetries < 1) {
        throw new Error(`Invalid max retries: ${maxRetries}. Max retries must be a positive integer.`);
    }
}
